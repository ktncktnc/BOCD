...
class SYCD(torch.utils.data.Dataset):
    ...
    def load_files(root: str, split: str):
        files = []
        images = glob(os.path.join(root, split, "time1", "*.png"))
        images = sorted([os.path.basename(image) for image in images])
        for image in images:
            image1 = os.path.join(root, split, "time1", image)
            image2 = os.path.join(root, split, "time2", image)
